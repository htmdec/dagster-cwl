...
# --------------------------------------------------------------------
# Resource: CWL runner configuration (modern, typed)
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# Config for the op (modern, typed)
# --------------------------------------------------------------------

class RunCWLConfig(Config):
    cwl_path: str                                    # Path or URL to the CWL workflow
    job_path: str                                    # Path to the CWL input object (YAML/JSON)
    outdir: str = "outputs"                          # Directory for workflow outputs
    cachedir: str = ".cwl-cache"                     # CWL cache
    extra_args: list[str] = []                       # Extra flags for the runner
    ...
    for name, val in outputs.items():
        if isinstance(val, dict) and val.get("class") == "File":
            ...
            meta = {
                "class": "Directory",
                "path": path,
                "listing_len": len(val.get("listing", [])) if isinstance(val.get("listing"), list) else None,
            }
            context.log_event(
                AssetMaterialization(
                    asset_key=AssetKey(["cwl_output", name]),
...
@op(
    config_schema={"output_key": Field(str, description="Name of the CWL output to select")},
...
def select_cwl_output(context: OpExecutionContext, outputs_map: dict) -> str:
    ...
    if not isinstance(path, str):
        raise Exception(f"Selected output '{key}' is not a file path (got {type(path).__name__}).")
